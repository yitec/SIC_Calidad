...
$consulta = "SELECT * FROM `vista_pendientes`  ORDER  BY `fecha_solicitud` ASC";	
    ...
    <div class="lista_titulo">Archivo</div><div class="lista_titulo">Comentario</div><div class="lista_titulo">Fecha</div></br></br></br></br>
     <?php				
									while($info=mysql_fetch_array($dt)){
										echo '
    <div class="lista">'.utf8_encode($info[1]).'</div><div class="lista">'.utf8_encode($info[2]).'</div><div class="lista">'.utf8_encode($info[3]).'</div><div class="lista_botones">
      <input name="button" id="button" value="Submit" type="image" src="img/btn_aprobar.png">
      <input  name="button2" id="button2" value="Submit" type="image" src="img/btn_rechazar.png">
